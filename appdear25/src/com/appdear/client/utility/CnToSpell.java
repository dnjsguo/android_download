...
//		bytes=utf82gbk(String.valueOf(cn));
	...
	public static byte[] utf82gbk(String chenese){ 
		  char c[] = chenese.toCharArray(); 
		  if(c==null||c.length==0)return null;
		         byte[] bytes=null;
				try {
					bytes = String.valueOf(c[0]).getBytes("utf-8");
			...
			if(bytes.length==3){
	           ...
	           for (int j = 0; j < bytes.length; j ++) {   
	                 // the first byte   
	        	   binaryStr.append(byte2bits(bytes[j]));
	           }
	           binaryStr.delete(16,18);
	           binaryStr.delete(8,10);
	           binaryStr.delete(0,4);
	           char c1=BinstrToChar(binaryStr.toString());
	           byte[] bs=String.valueOf(c1).getBytes();
		      return bs;
			}else{
				return  String.valueOf(c[0]).getBytes();
			}
		} 
		  ...
		  private static String byte2bits(byte b) { 
			  int z = b; z |= 256; String str = Integer.toBinaryString(z); int len = str.length(); 
