...
public class PointView extends TextView {
	...
	protected void onDraw(Canvas canvas) {
		...
		Bitmap greenPoint = BitmapFactory.decodeResource(getResources(), R.drawable.presence_online);
		Bitmap whitePoint = BitmapFactory.decodeResource(getResources(), R.drawable.presence_invisible);
	...
	public void refreshData(int totalPic, int currentPicPos) {
		 ...
		 */
		if (totalPic % 2 == 0) {  //偶数
			this.xStartPos = this.width / 2 - 
					(totalPic / 2 - 1) * (this.pointDiameter + this.pointDistance)  -
					this.pointDistance / 2 - this.pointRadius;
		} else { //基数
			this.xStartPos = this.width / 2 - 
					(totalPic / 2) * (this.pointDiameter + this.pointDistance);
		}
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		this.pointRadius = w / 32; //圆的半径，根据不同分辨率自适应
		this.pointDiameter = pointRadius * 2; //圆的直径
		this.pointDistance = w /17; //两个圆的间隔距离
